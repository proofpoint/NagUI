...
var NodeContextMenu= Ext.create('Ext.menu.Menu',{
	...
	items:[
		...
		{	
			...
			text: 'Acknowledge',
			setShow: function(n,v){
				if( !NagUI.nagios_write || ( n.data.nagios_type=='host' || n.data.nagios_type=='service') && n.data.state == 0 )
				{
					this.hide();
				}
			},
		...
		{	
			...
			text: 'Enable Alerts',
			setShow: function(n,v){
				if( !NagUI.nagios_write || ( n.data.nagios_type=='host' || n.data.nagios_type=='service') && n.data.notifications_enabled == 1 )
				{
					this.hide();
				}
			},
		...
		{	
			...
			text: 'Disable Alerts',
			setShow: function(n,v){
				if( !NagUI.nagios_write || ( n.data.nagios_type=='host' || n.data.nagios_type=='service') && n.data.notifications_enabled == 0 )
				{
					this.hide();
				}
			},
		...
		{	
			...
			setShow: function(n,v){
				if( !NagUI.nagios_write || n.data.downtimes_with_info == '' || n.data.downtimes_with_info.length == 0 )
				{
					this.hide();
				}
			},
