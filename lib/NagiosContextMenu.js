...
var NodeContextMenu= Ext.create('Ext.menu.Menu',{
	...
	items:[
		...
		{	
			...
			text: 'Acknowledge',
			setShow: function(n,v){
				if( !NagUI.nagios_write || !NagUI.config.enabled_actions.acknowledge 
					|| (n.data.nagios_type!='host' && n.data.nagios_type!='service') 
					|| n.data.state == 0 )
				{
					this.hide();
				}
		...
		{	
			...
			text: 'Enable Alerts',
			setShow: function(n,v){
				if( !NagUI.nagios_write || !NagUI.config.enabled_actions.enable_disable_alerts
					||( n.data.nagios_type!='host' && n.data.nagios_type!='service') 
					|| n.data.notifications_enabled == 1  )
				{
					this.hide();
				}
			},
		...
		{	
			...
			setShow: function(n,v){
				if( !NagUI.nagios_write || !NagUI.config.enabled_actions.enable_disable_alerts
					||( n.data.nagios_type!='host' && n.data.nagios_type!='service') 
					|| n.data.notifications_enabled == 0  )
				{
					this.hide();
				}
			},
		...
		{	
			...
			setShow: function(n,v){
				if( !NagUI.nagios_write || n.data.downtimes_with_info == '' 
					|| n.data.downtimes_with_info.length == 0 
					|| !NagUI.config.enabled_actions.scheduledowntime)
				{
					this.hide();
				}
			},
