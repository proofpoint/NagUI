...
NagUI.toggleLiveLog=function(logPanel){
	...
	{
		logPanel.liveTask.destroy();
		return;
	}
	logPanel.liveTask=Ext.util.TaskManager.start({
		run: function(){
			var time=Ext.Date.format(new Date(),'U')-100;
			if(this.store.max('time') > time)
			{
				time=this.store.max('time');	
			}
			this.getLogs({addRecords:true,rawquery: 'GET log|Columns: type host_name message lineno options contact_name service_description plugin_output state attempt class state_type time type command_name|Filter: state_type ~~ HARD|Filter: state_type ~~ DOWN|Or: 2|Filter: type ~~ SERVICE ALERT|Filter: state_type ~~ HARD|Filter: time >= ' + time });
		},
		scope: logPanel,
		interval: 5000
	});
